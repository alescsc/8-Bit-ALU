...
module ALU (
    input  wire       clk,        // Ceas
    input  wire       reset,      // Reset sistem
    input  wire       begin_sig,  // Start operatie
    input  wire [1:0] OP_MODE,    // Selectie operatie (00:+, 01:-, 10:*, 11:/)
    input  wire [7:0] INBUS,      // Intrare date
    output wire [7:0] OUTBUS      // Iesire rezultat
);

    // Semnale de control interne (generate de CU)
    wire C0, C1, C2, C3, C4, C5, C6, C7, C8, C9;
    wire C10, C11, C12, C13, C14, C15, C16, C17, C18;
    
    // Semnale de stare (trimise catre CU)
    wire Q1_out, Q0_out, R_out, OVR_out, A7_out, COUNT7;

    // Componenta pentru registre si calcule
    datapath DP (
    ...
    // Componenta pentru automatul de stari
    control_unit CU (
